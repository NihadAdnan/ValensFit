    ...
    public class MealBuilderService
    {
            ...
            double dailyTargetCarb)
        {
            ...
            var dayThemes = new[]
            {
                ...
                "Spartan Discipline & Fiber Cleanse",
            ...
            if (input.OfficeLunch)
            {
                // Portable simple lunch
                lunchPct = 0.22;
                breakfastPct = 0.39;
                dinnerPct = 0.39;
            }
            ...
            // Distinct vegetable rotation list as mentioned in PRD §4.6
            ...
            for (int dayIdx = 0; dayIdx < 7; dayIdx++)
            {
                ...
                var breakfast = BuildMeal(input, "Breakfast", "Morning Fortification", bTargetKcal, bTargetProtein, dayIdx, usedProteinHistory, usedVegHistory, null);
                ...
                var lunch = BuildMeal(input, "Lunch", input.OfficeLunch ? "Office Portable Fuel" : "Midday Power Feast", lTargetKcal, lTargetProtein, dayIdx, usedProteinHistory, usedVegHistory, preferredLunchVeg);
                ...
                var dinner = BuildMeal(input, "Dinner", "Evening Recovery Nourishment", dTargetKcal, dTargetProtein, dayIdx, usedProteinHistory, usedVegHistory, preferredDinnerVeg);
                ...
                // If sum is more than 4% off target, adjust dinner carbs/portions to bring day total within ±2%
            ...
            string? preferredVegId)
        {
            var meal = new MealPlanModel
            {
                ...
                TimingGuidance = slot switch
                {
                    ...
                    "Lunch" => "1:00 PM - 2:30 PM (Midday peak energy)",
            ...
            // 1. Pick Protein
            var proteinPool = _foodDb.FilterFoods(input.DietPreferences, input.CustomRestrictions, slot, "Protein");
            if (proteinPool.Count == 0) proteinPool = _foodDb.FilterFoods(null, null, slot, "Protein");
            ...
            // 2. Pick Carb
            var carbPool = _foodDb.FilterFoods(input.DietPreferences, input.CustomRestrictions, slot, "Carb");
            if (carbPool.Count == 0) carbPool = _foodDb.FilterFoods(null, null, slot, "Carb");
            ...
            {
                var vegPool = _foodDb.FilterFoods(input.DietPreferences, input.CustomRestrictions, slot, "Vegetable");
                if (vegPool.Count == 0) vegPool = _foodDb.FilterFoods(null, null, slot, "Vegetable");
            ...
            meal.CookingTip = $"{candidateProtein.PrepNotes} Pair with steamed {candidateVeg.Name} for optimal micronutrient absorption.";
